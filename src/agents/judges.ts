...
export async function evaluateIdeas(
  ...
  return runWithConcurrency(ideas, CONFIG.concurrency.llm, async (idea) => {
    ...
    const [officialRes, antiRes, demoRes] = await Promise.all([
      callAgent('judge_official', { ...baseVars, idea: ideaJson }, JudgeOfficialOutputSchema),
      callAgent('judge_anti_agent', { ...baseVars, idea: ideaJson }, JudgeAntiAgentOutputSchema),
      callAgent('judge_demo', { ...baseVars, idea: ideaJson }, JudgeDemoOutputSchema),
    ]);
